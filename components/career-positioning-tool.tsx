  ...
  return (
    <main className="min-h-screen bg-[#E5E3DC] text-[#303636]">
      <div className="mx-auto max-w-5xl p-4 md:p-8">
        <header className="mb-6 rounded-xl border border-[#495A58]/20 bg-white p-6 shadow-sm">
          <p className="font-ui text-sm text-[#495A58]">American Health Equity Association · AI Tools</p>
          <h1 className="font-heading mt-3 text-4xl">Career Positioning Tool</h1>
          <p className="mt-4">Reframe your experience and communicate your value for evolving roles, sectors, and opportunities in a rapidly changing public health and health equity landscape.</p>
          <p className="mt-3">Use this tool to strengthen career language for resumes, LinkedIn profiles, bios, interviews, networking, consulting, and transitions. It is designed for public health and health equity professionals who need to clarify their value and adapt their experience for what comes next.</p>
        </header>
        <section className="mb-6 rounded-xl border border-[#495A58]/20 bg-white p-5">
          <h2 className="font-heading text-2xl">Access and usage</h2>
          <p className="mt-2 text-sm">{me?.message ?? "Your access and generation availability are managed through the shared AHEA backend."}</p>
          {typeof me?.freeGenerationsRemaining === "number" && <p className="mt-2 text-sm">Complimentary generations remaining: {me.freeGenerationsRemaining}</p>}
          {blocked && (
        ...
        <form onSubmit={onSubmit} className="rounded-xl border border-[#495A58]/20 bg-white p-6">
          {errors.form && <p role="alert" className="mb-4 rounded border border-[#D4967D] bg-[#E5E3DC] p-2">{errors.form}</p>}
          <div className="grid gap-4">
            <label className="block"><span>What are you working on? *</span><select className="mt-1 w-full rounded border p-2" value={input.outputType} onChange={(e) => setField("outputType", e.target.value as CareerPositioningInput["outputType"])}><option value="resume_summary">Resume summary</option><option value="linkedin_about">LinkedIn About section</option><option value="professional_bio">Professional bio</option><option value="interview_networking">Interview or networking language</option><option value="career_transition">Career transition language</option><option value="consulting_independent">Consulting or independent practice language</option></select></label>
            <label className="block"><span>Paste your current language or rough notes *</span><textarea className="mt-1 min-h-40 w-full rounded border p-2" value={input.currentLanguage} onChange={(e) => setField("currentLanguage", e.target.value)} placeholder="Example: I’ve worked in public health for several years..." />{errors.currentLanguage && <p className="text-sm text-red-700">{errors.currentLanguage}</p>}</label>
            <label className="block"><span>What best describes your current or recent work? *</span><select className="mt-1 w-full rounded border p-2" value={input.currentWork} onChange={(e) => setField("currentWork", e.target.value as CareerPositioningInput["currentWork"])}><option value="public_health_programs">Public health programs</option><option value="health_equity_community_health">Health equity or community health</option><option value="research_data_evaluation">Research, data, or evaluation</option><option value="policy_advocacy_systems_change">Policy, advocacy, or systems change</option><option value="communications_strategy_public_affairs">Communications, strategy, or public affairs</option><option value="healthcare_population_health_social_care">Healthcare, population health, or social care</option><option value="nonprofit_philanthropy_community_based">Nonprofit, philanthropy, or community-based work</option><option value="other_mixed">Other or mixed background</option></select></label>
            <label className="block"><span>What are you trying to move toward? *</span><select className="mt-1 w-full rounded border p-2" value={input.desiredDirection} onChange={(e) => setField("desiredDirection", e.target.value as CareerPositioningInput["desiredDirection"])}><option value="similar_field">A new role in a similar field</option><option value="different_sector">A role in a different sector</option><option value="leadership_role">A leadership role</option><option value="consulting_independent">Consulting or independent work</option><option value="broader_public_health_strategy_systems">Broader public health, strategy, or systems work</option><option value="not_sure">I’m not sure yet</option></select></label>
            ...
            <label className="block"><span>What kind of professional context are you preparing for? *</span><select className="mt-1 w-full rounded border p-2" value={input.professionalContext} onChange={(e) => setField("professionalContext", e.target.value as CareerPositioningInput["professionalContext"])}><option value="direct_values_forward">Direct and values-forward</option><option value="balanced_broadly_accessible">Balanced and broadly accessible</option><option value="careful_institutionally_appropriate">Careful and institutionally appropriate</option><option value="cross_sector_unfamiliar_audience">Cross-sector or unfamiliar audience</option><option value="recommend_best_fit">I’m not sure — recommend the best fit</option></select></label>
            <label className="block"><span>Anything to emphasize, avoid, or explain?</span><textarea className="mt-1 min-h-28 w-full rounded border p-2" value={input.additionalContext} onChange={(e) => setField("additionalContext", e.target.value)} />{errors.additionalContext && <p className="text-sm text-red-700">{errors.additionalContext}</p>}</label>
          </div>
          <button disabled={loading || blocked} className="mt-6 rounded bg-[#D4967D] px-5 py-2 font-ui disabled:opacity-60">{loading ? "Generating your career positioning language…" : "Generate career positioning language"}</button>
          {serverMessage && <p className="mt-3" role="status">{serverMessage}</p>}
        </form>
...
function OutputCard({ title, children, onCopy, copied }: { title: string; children: React.ReactNode; onCopy: () => void; copied: boolean }) {
  return <article className="rounded-xl border border-[#495A58]/20 bg-white p-5"><div className="mb-3 flex items-center justify-between gap-3"><h3 className="font-heading text-2xl">{title}</h3><button onClick={onCopy} className="rounded border px-3 py-1 font-ui" aria-label={`Copy ${title}`}>{copied ? "Copied" : "Copy"}</button></div><div className="space-y-3">{children}</div></article>;
}
